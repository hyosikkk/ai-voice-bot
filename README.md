...
- **다국어 지원**: 한국어, 영어, 일본어, 스페인어, 프랑스어, 독일어, 중국어, 아랍어, 이탈리아어, 포르투갈어 (10개 언어)
- **다양한 파일 형식**: MP3, WAV, MP4, WebM 등 오디오/비디오 파일 지원 (최대 500MB)
- **비디오 더빙 비교**: 비디오 업로드 시 원본 영상과 더빙 영상을 나란히 동시 재생하여 비교
- **더빙 영상 다운로드**: ffmpeg.wasm을 활용해 브라우저에서 원본 영상 + 더빙 음성을 합쳐 MP4로 다운로드
- **발화 구간별 싱크 조정**: ElevenLabs 캐릭터 정렬 데이터 기반으로 각 발화 구간마다 개별 atempo 적용, 구간 경계마다 오디오 위치 하드싱크
- **자막 동기화**: STT 단어 타임스탬프로 자막 세그먼트 자동 생성, 비디오 위에 실시간 오버레이 (원본/번역/동시 표시 전환 가능)
- **SRT 다운로드**: 원본·번역 자막을 SRT 파일로 개별 다운로드, MP4 다운로드 시 소프트 자막 트랙 자동 임베드
- **자유 구간 크롭**: 비디오/오디오 업로드 전 시작·끝 구간을 자유롭게 선택하여 원하는 부분만 더빙
...
```
파일 업로드 (오디오/비디오, 최대 500MB)
        ↓
[선택] ffmpeg.wasm — 자유 구간 크롭 (브라우저)
        ↓
ElevenLabs Scribe v1 — 음성 추출 + 단어 타임스탬프 (STT)
        ↓
단어 타임스탬프 → 자막 세그먼트 생성 (문장 부호 / 무음 0.8s / 최대 5s 기준 분할)
        ↓
Google Cloud Translation API — 세그먼트별 배치 번역
        ↓
ElevenLabs Multilingual v2 with-timestamps — 더빙 생성 + 캐릭터 정렬 (TTS)
        ↓
캐릭터 정렬 → 세그먼트별 SyncMap 계산 (origStart/dubStart/rate)
        ↓
결과물 출력
  ├── 오디오: MP3 재생 및 다운로드 + SRT 다운로드
  └── 비디오: 원본/더빙 영상 나란히 비교 재생
              ├── 자막 오버레이 (원본/번역/동시 전환)
              ├── 세그먼트별 개별 싱크 (구간 경계 하드싱크)
              └── MP4 다운로드 (filter_complex 세그먼트 atempo + soft-sub 임베드)
```
...
### v3 — 자막 동기화 · 세밀 싱크 · 언어 확장 (2026-03-23)

| 분류 | 내용 |
|------|------|
| **자막 동기화** | ElevenLabs STT 단어 타임스탬프 기반 자막 세그먼트 자동 생성, 비디오 위 실시간 오버레이 |
| **자막 표시 모드** | 원본 / 번역 / 원본+번역 / 끄기 전환 버튼 |
| **SRT 다운로드** | 원본·번역 자막 각각 `.srt` 파일 다운로드 |
| **MP4 소프트 자막** | 더빙 MP4 다운로드 시 번역 자막을 `mov_text` 트랙으로 임베드 |
| **발화 구간별 싱크** | ElevenLabs `/with-timestamps` 캐릭터 정렬로 세그먼트별 rate 계산, 구간 경계마다 하드싱크 |
| **ffmpeg filter_complex** | 다운로드 시 세그먼트별 `atrim → atempo → adelay → amix` 개별 적용 |
| **지원 언어 10개** | 프랑스어, 독일어, 중국어, 아랍어, 이탈리아어, 포르투갈어 추가 |
| **번역 최적화** | 전체 텍스트/세그먼트 분리 번역 → 세그먼트 배치 번역 단일화 (API 호출 감소) |

### v2 — 영상 크롭 · 대용량 파일 (이전)

| 분류 | 내용 |
|------|------|
| **자유 구간 크롭** | ffmpeg.wasm으로 시작·끝 구간 자유 선택 후 더빙 |
| **파일 크기 확장** | 최대 100MB → 500MB |
| **모바일 최적화** | 크롭 UI 터치 영역 확대 |
| **playbackRate 싱크** | 더빙 음성 길이 / 영상 길이 비율로 전역 속도 자동 조정 |
